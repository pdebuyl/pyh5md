...
def populate_H5MD_data(g, name, shape, dtype, chunks=None, N_fixed=True, fillvalue=None):
    ...
    if fillvalue is not None:
        g.value = g.create_dataset('value', shape=(0,)+shape, dtype=dtype, maxshape=maxshape, chunks=chunks, fillvalue=fillvalue)
    else:
        ...
        if name in parent.keys():
            ...
            if data is not None:
                if shape is not None or isinstance(dtype, np.dtype):
                    ...
                    populate_H5MD_data(self, name, data.shape, data.dtype, chunks=chunks, N_fixed=N_fixed, fillvalue=fillvalue)
            else:
                self._id = h5py.h5g.create(parent.id, name)
